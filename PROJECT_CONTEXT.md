...
- **遗言 / 死亡结算**：Towel.Die → AIAgent.OnStageDeathAsync（击杀横幅 + 遗言请求；**没有超时兜底**，请求失败或回复为空就不显示遗言）→ 死亡**瞬间**倒序记录并删除**自己阵营**的弹珠（别人的弹珠不动），遗言说完后逐颗放成大球 → 道具栏**按道具本体释放**（大球放球、穿甲放穿甲弹、霰弹放霰弹；护盾/扫射/【任意】放大球）→ 炮塔自身数值与残余子弹各放成一颗球（子弹是收掉后合并）
