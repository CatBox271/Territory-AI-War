...
public class ForceShake : MonoBehaviour
{
    ...
    public class Shake
    {
        ...
        public Vector2 force = new(0, 1);//运动方向
    ...
    public IEnumerator Shaking(Shake shake)
    {
        ...
        while (time < shake.finish_time)
        {
            if (time < shake.mid_time)
            {
                ...
                {
                    _pos = (Vector3)shake.force * (tt ? 1 : -1) * shake.intensity * (1 + time / shake.mid_time);
            ...
            else
            {
                if (damp_time >= shake.damp)
                {
                    _pos = (Vector3)shake.force * (tt ? 1 : -1) * shake.intensity * ((shake.finish_time - time) / (shake.finish_time - shake.mid_time));
                    damp_time -= shake.damp;
                    tt = !tt;
                }
            }
