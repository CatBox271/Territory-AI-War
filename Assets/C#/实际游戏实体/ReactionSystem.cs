...
public class ReactionSystem : MonoBehaviour, Itool
{
    ...
    public List<Tool> tools = new List<Tool>
    {
        ...
        new Tool
        {
            ...
            function = new Function
            {
                ...
                description = "移动自己的炮塔（两步确认）：第一次只预览、不会移动——给方向（angle 角度，或 dir_x+dir_y 向量，会自动归一化）与 distance 距离，工具算出目标点并返回；看清返回里的目标点和警告后，第二次调用不再给方向距离，只传 confirm=true 才会真正开始移动。每次移动消耗升级能量（与距离无关，具体数值见预览返回；同一个动作每用一次价格 ×涨价倍率，越移越贵），能量不足则**连预览都不给**，也就不会附带视野截图。炮塔按固定速度直线移动，到达目标、撞到地图边界或超时会停下并通知你；distance=0 表示原地不动（用来停下）。**移动途中可以改道**：再发一次预览 + confirm 就会立刻朝新方向走（改道同样按当前价格扣一次能量）。移动本身不会主动广播你的新位置（对方要么靠大球/子弹撞上你护盾或炮塔本体时的撞击情报反推，要么你正好落进他 move_turret 预览附带的视野截图里被看到）。距离由你自己定：最大移动距离是上限、不是必须走满；确认前先核对目标点是否靠近各炮塔初始位置或撞击情报推测出的敌方方位，炮塔重叠不会被弹开、也不会自动停下，停到别人身上等于把命送出去。预览时还会附一张**以地图中心 (0,0) 为中心、完整覆盖整张地图的正方形俯视图**（比例固定：整张地图正好铺满这张图，所以图上可以直接按坐标估距离）。图上**亮着的部分 = 你看得见的范围 = 以你炮塔为圆心、半径 = 你最大移动距离 × 截图半径倍率的那个圆 ∪ 你自己的领土**（领土边界外还多露一点，方便看清接壤的是谁）；**其余全是纯黑 = 你看不见的地方**（那里有没有东西、是谁的地，图上一律没有信息）。**图例：图上标注一律是黑色字**（阵营写在字里，不靠颜色认）；每个实体只有**一条**标签，内容是「阵营 + 类别 + 数值 + 坐标 + 速度方向」（炮塔只写阵营+类别+坐标、不标数值）；标记本体：圆环 = 大球（环的大小就是球的实际大小）、叉 = 穿甲弹、圆点 = 炮塔，带箭头的粗线 = 它正在飞的方向。**看得见的范围 = 以你为圆心的那一圈 ∪ 你自己的领土**：圈内看得见；圈外**只有你自己领土上**的东西看得见；再外面被**压暗**——压暗区仍保留那块地的归属色（红方的地是红、绿方的地是绿，看得出是谁的地），但看不清上面有什么。**；**图里看得见的东西另外还会用文字列一份坐标**（谁的什么、在哪个方位几格、世界坐标约多少），要用道具打谁就直接照那份文字里的坐标瞄；先看图再决定方向和距离。",
                parameters = new
                {
                    type = "object",
                    additionalProperties = false,
    ...
    /// <summary>
    /// 给这次移动预览附一张「整张地图（以 (0,0) 为中心）的俯视图」，并按可见范围抹黑。
    ...
    private static void AppendMoveSight(List<DeepSeekMessage> messages, int callStage)
    {
        ...
        string text =
            $"【移动视野】这是**以地图中心 (0,0) 为中心、完整覆盖整张地图的正方形俯视图**" +
            $"（地图 = x,y ∈ [{-MapConfig.Instance.worldSize * 0.5f:0.#}, {MapConfig.Instance.worldSize * 0.5f:0.#}]；" +
            "比例固定：整张地图正好铺满这张图，所以图上可以直接按坐标估距离）。" +
            $"图上**亮着的部分 = 你看得见的范围**：以你炮塔（当前 {towel.transform.position.x:0.0},{towel.transform.position.y:0.0}）为圆心、" +
            $"半径 {shot.radius:0.00} 的圆（= 你当前最大移动距离 {shot.maxMove:0.00} 的 {MoveSightCapture.RadiusFactor:0.##} 倍）" +
            "**∪ 你自己的领土**（领土边界还向外多露了一点，方便你看清**接壤的是谁**）；**其余全是纯黑 = 你看不见的地方**（那里有没有东西、谁的地，图上一律没有信息）。" +
            "**图例：图上标注一律是黑色字**（阵营写在字里，不靠颜色认）；每个实体只有**一条**标签，内容是「阵营 + 类别 + 数值 + 坐标 + 速度方向」（炮塔只写阵营+类别+坐标、不标数值）；标记本体：圆环 = 大球（环的大小就是球的实际大小）、叉 = 穿甲弹、圆点 = 炮塔，带箭头的粗线 = 它正在飞的方向；只标**中心点落在看得见范围内**的实体（敌人和自己的炮塔都算）。" +
            "图片上方 = +y（北），右方 = +x（东），图片正中心 = 世界 (0,0)。" +
