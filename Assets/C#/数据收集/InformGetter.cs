    ...
    private static void AppendBallImpactWarning(StringBuilder builder, int stage)
    {
        ...
        foreach (var kv in Oitems)
        {
            ...
            foreach (ItemType item in kv.Value)
            {
                ...
                // 换位方向还要避开大球：2026-09-22 用户看视野图时发现 AI"向左躲掉了但差点撞到球上"。
                // 这里只写机制（撞上会怎样、球的位置在哪能看），不写劝告。
                builder.Append("**换位方向还要避开场上的大球**：撞上大球会按 bounceRate 结算一次撞击、并且你的速度会被它带偏"
                             + "（大球的位置、数值、速度就在这一轮的视野图与【图里看见的】清单里）。");
